...
class Auth extends MY_Controller {
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
				...
				if ($user->group_id === "1") {
					...
					if($this->session->has_userdata('sesi_pengunjung')){
						redirect('investor/payment/process', 'refresh');
					} else {
						redirect('investor/home','refresh');
					}
				} else {
	...
	public function create_user()
	{
		...
		if ($this->form_validation->run() == true && $this->ion_auth->register($identity, $password, $email, $additional_data))
		{
			...
			$this->message('Silahkan masuk menggunakan akun yang sudah anda daftarkan', 'primary');
			redirect("auth/login", 'refresh');
		}
		else
		{
			...
			$this->data['password_confirm'] = array(
				...
				'type'  => 'password',
				'class' => 'form-control',
				'placeholder' => 'Ulangi Penulisan Password',
